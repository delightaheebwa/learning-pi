...
Begin every assistant message with a turn tag on its first line — `[[TURN:quiz]]` (exit ticket), `[[TURN:grade]]` (grading), or `[[TURN:none]]` (summaries). The gate strips the tag before the learner sees it. A dropped grade/quiz tag is tolerated while its verifier receipt is pending; `claims`/`none` must be tagged.
