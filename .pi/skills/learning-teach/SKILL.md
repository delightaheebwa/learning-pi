...
- **Mini-checkpoint pause protocol (mandatory — the learner is never rushed):** deliver one mini-checkpoint at a time and never chain mini-checkpoint → practice → next checkpoint. Run this exact loop for every checkpoint:
  1. **Mini-checkpoint idea** (`[[TURN:claims]]`, fact-checked) — teach ONE atomic reasoning unit. Do **not** fold the checkpoint's other mini-checkpoints into this message. If it is growing past one idea + one implication, split it.
